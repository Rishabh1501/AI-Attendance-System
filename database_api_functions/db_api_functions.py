...
class DatabaseAPI:
    ...
    def make_database_collection(self,collection_name=None):
        ...
        if collection_name in collection_list:
            ...
            print(f"Collection:'{collection_name}' does not exists OR no documents are present in the collectionin Database")
        ...
        if not self.collection.find_one({'Name':name}):
            ...
            if(self.collection.find_one({'Name':name,'Check Out':1})):
                checkin_status = "You Already Checked Out at " + \
                    str(self.collection.find_one({'Name':name})["Time"]) + " ! See You Tomorrow :)"
    ...
    def check_out(self, name, save_image=True):
        ...
        if not self.collection.find_one({'Name':name}):
            ...
            if(self.collection.find_one({'Name':name,'Check Out':1})):
                ...
                time_now = datetime.now()
                self.collection.update_one(self.collection.find_one({'Name':name}),{"$set":{'Check Out': 1,'Check Out Time': time_now.strftime('%H:%M:%S')}})
